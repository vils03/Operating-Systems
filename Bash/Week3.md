...
#### 17. Копирайте /srv/fmi-os/exercises/data/emp.data във вашата home директория.
...
```shell
$ cat dir5/file1 dir5/file2 >> concat && cat concat dir5/file3 >> cat123.txt
...
```shell
$ awk '{print tolower($0)}' dir5/file1 > dir5/file2
...
```shell
$ cat /etc/passwd|head -n 46 |tail -n +28|awk -F : '{print $3}'|rev|cut -c1
...
```shell
$ find /tmp -type f -printf '%M\t%p' -readable 2>/dev/null 
...
```shell
$ find /usr/include -maxdepth 1 -type f -name '*[.c,.h]'|wc -l
$ find /usr/include -maxdepth 1 -type f -name '*[.c,.h]'|xargs wc -l {}
